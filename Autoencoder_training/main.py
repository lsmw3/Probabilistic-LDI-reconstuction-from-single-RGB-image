...
# use tensor cores for fp32 as well
...
class ImageLogger(Callback):
    ...
    def log_local(self, save_dir, split, images,
        ...
        for k in images:
            if True:
                numpy_images = images[k].numpy()
                filename = "{}_b-{:06}.npy".format(
                    k,
                    batch_idx)
                
                path = os.path.join(root, filename)
                os.makedirs(os.path.split(path)[0], exist_ok=True)
                np.save(path, numpy_images)
    ...
    def log_img(self, pl_module, batch, batch_idx, split="train"):
        ...
        if ((split=="val" or split=="test") and  # batch_idx % self.batch_freq == 0
